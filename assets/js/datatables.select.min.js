...
if(opts.headerCheckbox!==undefined){headerCheckbox=opts.headerCheckbox}}
dt.select.selector(selector);dt.select.items(items);dt.select.style(style);dt.select.blurable(blurable);dt.select.toggleable(toggleable);dt.select.info(info);ctx._select.className=className;if(!setStyle&&$(dt.table().node()).hasClass('selectable')){dt.select.style('os')}
if(headerCheckbox||headerCheckbox==='select-page'||headerCheckbox==='select-all'){initCheckboxHeader(dt,headerCheckbox);dt.on('init',function(){initCheckboxHeader(dt,headerCheckbox)})}};function cellRange(dt,idx,last){var indexes;var columnIndexes;var rowIndexes;var selectColumns=function(start,end){if(start>end){var tmp=end;end=start;start=tmp}
...
function checkboxClass(selector){var name=DataTable.select.classes.checkbox;return selector?name.replace(/ /g,'.'):name}
...
var ctx=dt.settings()[0];var container=dt.table().container();if($(e.target).closest('div.dt-container')[0]!=container){return}
...
function info(api,node){if(api.select.style()==='api'||api.select.info()===!1){return}
var rows=api.rows({selected:!0}).flatten().length;var columns=api.columns({selected:!0}).flatten().length;var cells=api.cells({selected:!0}).flatten().length;var add=function(el,name,num){el.append($('<span class="select-item"/>').append(api.i18n('select.'+name+'s',{_:'%d '+name+'s selected',0:'',1:'1 '+name+' selected'},num)))};var el=$(node);var output=$('<span class="select-info"/>');add(output,'row',rows);add(output,'column',columns);add(output,'cell',cells);var existing=el.children('span.select-info');if(existing.length){existing.remove()}
if(output.text()!==''){el.append(output)}}
function initCheckboxHeader(dt,headerCheckbox){dt.columns('.dt-select').every(function(){var header=this.header();if(!$('input',header).length){var input=$('<input>').attr({class:checkboxClass(!0),type:'checkbox','aria-label':dt.i18n('select.aria.headerCheckbox')||'Select all rows'}).appendTo(header).on('change',function(){if(this.checked){if(headerCheckbox=='select-page'){dt.rows({page:'current'}).select()}else{dt.rows({search:'applied'}).select()}}else{dt.rows({selected:!0}).deselect()}}).on('click',function(e){e.stopPropagation()});dt.on('draw select deselect',function(e,pass,type){if(type==='row'||!type){var count=dt.rows({selected:!0}).count();var search=dt.rows({search:'applied',selected:!0}).count();var available=headerCheckbox=='select-page'?dt.rows({page:'current'}).count():dt.rows({search:'applied'}).count();if(search&&search<=count&&search===available){input.prop('checked',!0).prop('indeterminate',!1)}else if(search===0&&count===0){input.prop('checked',!1).prop('indeterminate',!1)}else{input.prop('checked',!1).prop('indeterminate',!0)}}})}})}
function init(ctx){var api=new DataTable.Api(ctx);ctx._select_init=!0;ctx.aoRowCreatedCallback.push(function(row,data,index){var i,ien;var d=ctx.aoData[index];if(d._select_selected){$(row).addClass(ctx._select.className).find('input.'+checkboxClass(!0)).prop('checked',!0)}
for(i=0,ien=ctx.aoColumns.length;i<ien;i++){if(ctx.aoColumns[i]._select_selected||(d._selected_cells&&d._selected_cells[i])){$(d.anCells[i]).addClass(ctx._select.className)}}});api.on('preXhr.dt.dtSelect',function(e,settings){if(settings!==api.settings()[0]){return}
var rows=api.rows({selected:!0}).ids(!0).filter(function(d){return d!==undefined});var cells=api.cells({selected:!0}).eq(0).map(function(cellIdx){var id=api.row(cellIdx.row).id(!0);return id?{row:id,column:cellIdx.column}:undefined}).filter(function(d){return d!==undefined});api.one('draw.dt.dtSelect',function(){api.rows(rows).select();if(cells.any()){cells.each(function(id){api.cells(id.row,id.column).select()})}})});api.on('info.dt',function(e,ctx,node){if(!ctx._select.infoEls.includes(node)){ctx._select.infoEls.push(node)}
info(api,node)});api.on('select.dtSelect.dt deselect.dtSelect.dt',function(){ctx._select.infoEls.forEach(function(el){info(api,el)});api.state.save()});api.on('destroy.dtSelect',function(){$(api.rows({selected:!0}).nodes()).removeClass(api.settings()[0]._select.className);disableMouseSelection(api);api.off('.dtSelect');$('body').off('.dtSelect'+_safeId(api.table().node()))})}
function rowColumnRange(dt,type,idx,last){var indexes=dt[type+'s']({search:'applied'}).indexes();var idx1=indexes.indexOf(last);var idx2=indexes.indexOf(idx);if(!dt[type+'s']({selected:!0}).any()&&idx1===-1){indexes.splice(indexes.indexOf(idx)+1,indexes.length)}else{if(idx1>idx2){var tmp=idx2;idx2=idx1;idx1=tmp}
indexes.splice(idx2+1,indexes.length);indexes.splice(0,idx1)}
if(!dt[type](idx,{selected:!0}).any()){dt[type+'s'](indexes).select()}else{indexes.splice(indexes.indexOf(idx),1);dt[type+'s'](indexes).deselect()}}
...
for(var i=0,ien=indexes.length;i<ien;i++){data=settings[o.prop][indexes[i]];if(data&&((selected===!0&&data._select_selected===!0)||(selected===!1&&!data._select_selected))){out.push(indexes[i])}}
...
for(var i=0,ien=cells.length;i<ien;i++){rowData=settings.aoData[cells[i].row];if(rowData&&((selected===!0&&rowData._selected_cells&&rowData._selected_cells[cells[i].column]===!0)||(selected===!1&&(!rowData._selected_cells||!rowData._selected_cells[cells[i].column])))){out.push(cells[i])}}
...
return this.iterator('table',function(ctx){disableMouseSelection(new DataTable.Api(ctx));ctx._select.selector=selector;if(ctx._select.style!=='api'){enableMouseSelection(new DataTable.Api(ctx))}})});apiRegister('select.last()',function(set){let ctx=this.context[0];if(set){ctx._select_lastCell=set;return this}
return ctx._select_lastCell});apiRegisterPlural('rows().select()','row().select()',function(select){var api=this;if(select===!1){return this.deselect()}
this.iterator('row',function(ctx,idx){clear(ctx);var dtData=ctx.aoData[idx];var dtColumns=ctx.aoColumns;$(dtData.nTr).addClass(ctx._select.className);dtData._select_selected=!0;for(var i=0;i<dtColumns.length;i++){var col=dtColumns[i];if(col.sType===null){api.columns().types()}
if(col.sType==='select-checkbox'){var cells=dtData.anCells;if(cells&&cells[i]){$('input.'+checkboxClass(!0),cells[i]).prop('checked',!0)}
if(dtData._aSortData!==null){dtData._aSortData[i]=null}}}});this.iterator('table',function(ctx,i){eventTrigger(api,'select',['row',api[i]],!0)});return this});apiRegister('row().selected()',function(){var ctx=this.context[0];if(ctx&&this.length&&ctx.aoData[this[0]]&&ctx.aoData[this[0]]._select_selected){return!0}
...
return!1});apiRegisterPlural('rows().deselect()','row().deselect()',function(){var api=this;this.iterator('row',function(ctx,idx){var dtData=ctx.aoData[idx];var dtColumns=ctx.aoColumns;$(dtData.nTr).removeClass(ctx._select.className);dtData._select_selected=!1;ctx._select_lastCell=null;for(var i=0;i<dtColumns.length;i++){var col=dtColumns[i];if(col.sType===null){api.columns().types()}
if(col.sType==='select-checkbox'){var cells=dtData.anCells;if(cells&&cells[i]){$('input.'+checkboxClass(!0),dtData.anCells[i]).prop('checked',!1)}
if(dtData._aSortData!==null){dtData._aSortData[i]=null}}}});this.iterator('table',function(ctx,i){eventTrigger(api,'deselect',['row',api[i]],!0)});return this});apiRegisterPlural('columns().deselect()','column().deselect()',function(){var api=this;this.iterator('column',function(ctx,idx){ctx.aoColumns[idx]._select_selected=!1;var api=new DataTable.Api(ctx);var column=api.column(idx);$(column.header()).removeClass(ctx._select.className);$(column.footer()).removeClass(ctx._select.className);api.cells(null,idx).indexes().each(function(cellIdx){var data=ctx.aoData[cellIdx.row];var cellSelected=data._selected_cells;if(data.anCells&&(!cellSelected||!cellSelected[cellIdx.column])){$(data.anCells[cellIdx.column]).removeClass(ctx._select.className)}})});this.iterator('table',function(ctx,i){eventTrigger(api,'deselect',['column',api[i]],!0)});return this});apiRegisterPlural('cells().deselect()','cell().deselect()',function(){var api=this;this.iterator('cell',function(ctx,rowIdx,colIdx){var data=ctx.aoData[rowIdx];if(data._selected_cells!==undefined){data._selected_cells[colIdx]=!1}
...
function enabled(dt,config){if(config.limitTo.indexOf('rows')!==-1&&dt.rows({selected:!0}).any()){return!0}
if(config.limitTo.indexOf('columns')!==-1&&dt.columns({selected:!0}).any()){return!0}
if(config.limitTo.indexOf('cells')!==-1&&dt.cells({selected:!0}).any()){return!0}
return!1}
...
this[items+'s'](mod).select()}else{this[items+'s']().select()}}},selectNone:{text:i18n('selectNone','Deselect all'),className:'buttons-select-none',action:function(){clear(this.settings()[0],!0)},init:function(dt,node,config){var that=this;config._eventNamespace='.select'+_buttonNamespace++;dt.on(namespacedEvents(config),function(){var count=dt.rows({selected:!0}).flatten().length+dt.columns({selected:!0}).flatten().length+dt.cells({selected:!0}).flatten().length;that.enable(count>0)});this.disable()},destroy:function(dt,node,config){dt.off(config._eventNamespace)}},showSelected:{text:i18n('showSelected','Show only selected'),className:'buttons-show-selected',action:function(e,dt){if(dt.search.fixed('dt-select')){dt.search.fixed('dt-select',null);this.active(!1)}else{var dataSrc=dt.settings()[0].aoData;dt.search.fixed('dt-select',function(text,data,idx){return dataSrc[idx]._select_selected});this.active(!0)}
dt.draw()}}});$.each(['Row','Column','Cell'],function(i,item){var lc=item.toLowerCase();DataTable.ext.buttons['select'+item+'s']={text:i18n('select'+item+'s','Select '+lc+'s'),className:'buttons-select-'+lc+'s',action:function(){this.select.items(lc)},init:function(dt){var that=this;dt.on('selectItems.dt.DT',function(e,ctx,items){that.active(items===lc)})}}});DataTable.type('select-checkbox',{className:'dt-select',detect:function(data){return data==='select-checkbox'?data:!1},order:{pre:function(d){return d==='X'?-1:0}}});$.extend(!0,DataTable.defaults.oLanguage,{select:{aria:{rowCheckbox:'Select row'}}});DataTable.render.select=function(valueProp,nameProp){var valueFn=valueProp?DataTable.util.get(valueProp):null;var nameFn=nameProp?DataTable.util.get(nameProp):null;return function(data,type,row,meta){var dtRow=meta.settings.aoData[meta.row];var selected=dtRow._select_selected;var ariaLabel=meta.settings.oLanguage.select.aria.rowCheckbox;if(type==='display'){return $('<input>').attr({'aria-label':ariaLabel,class:checkboxClass(),name:nameFn?nameFn(row):null,type:'checkbox',value:valueFn?valueFn(row):null,checked:selected}).on('input',function(e){e.preventDefault();this.checked=$(this).closest('tr').hasClass('selected')})[0]}else if(type==='type'){return'select-checkbox'}else if(type==='filter'){return''}
return selected?'X':''}}
DataTable.ext.order['select-checkbox']=function(settings,col){return this.api().column(col,{order:'index'}).nodes().map(function(td){if(settings._select.items==='row'){return $(td).parent().hasClass(settings._select.className)}else if(settings._select.items==='cell'){return $(td).hasClass(settings._select.className)}
return!1})};$.fn.DataTable.select=DataTable.select;$(document).on('preInit.dt.dtSelect',function(e,ctx){if(e.namespace!=='dt'){return}
